...
static CHAR_STATE char_parser_readnum(char_parser_t* p) {
	...
	if(p->ls->tk_type != TK_NUMBER) {
		char_err_rdrprint(CHAR_ERR_T_SYNTAX, p->ls->rdr, 0, CHAR_ERR_NUM_EXP);
		return CHAR_ERROR;
	}
	...
	switch(p->cmd) {
		...
		case ';': {
			...
			if(p->ls->tk_type != TK_IDENT) {
				char_err_rdrprint(CHAR_ERR_T_SYNTAX, p->ls->rdr, 0, CHAR_ERR_LBL_EXP);
				return CHAR_ERROR;
			}
		...
		case TK_IDENT:
			char_err_rdrprint(CHAR_ERR_T_SYNTAX, p->ls->rdr, p->ls->tk_lexeme->len, CHAR_ERR_IDENT_UNEXP, p->ls->tk_lexeme->src);
			return CHAR_ERROR;
		case TK_NUMBER:
			char_err_rdrprint(CHAR_ERR_T_SYNTAX, p->ls->rdr, p->ls->tk_lexeme->len, CHAR_ERR_NUM_UNEXP, p->ls->tk_lexeme->src);
			return CHAR_ERROR;
	}
