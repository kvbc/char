...
char_vm_state_t* char_vm_new(void) {
	char_vm_state_t* vm = malloc(sizeof(char_vm_state_t));
	vm->err    = NULL;
...
static CHAR_STATE char_vm_runstmt(char_vm_state_t* vm) {
	switch(vm->p->cmd) {
		...
		case ';': {
			...
			if(!lbl) {
				char_err_rdrprint(CHAR_ERR_T_RUNTIME, vm->ls->rdr, vm->p->v.s->len, CHAR_ERR_LBL_UND, vm->p->v.s->src);
				return CHAR_ERROR;
			}
			if(vm->p->cmd == ';') {
		...
		case '@':
			return CHAR_ERROR;
		default:
			char_err_rdrprint(CHAR_ERR_T_SYNTAX, vm->ls->rdr, 1, CHAR_ERR_CHAR_UNEXP, vm->p->cmd);
			return CHAR_ERROR;
	}
...
void char_vm_run(char_vm_state_t* vm, char* src, char* inp) {
	...
	do {
		...
		if(ls->tk_type == '^') {
			...
			if(ls->tk_type != TK_IDENT) {
				char_err_rdrprint(CHAR_ERR_T_SYNTAX, ls->rdr, 0, CHAR_ERR_LBL_EXP);
				char_lex_close(ls);
				return;
			}
			if(char_loctab_lookup(vm->lbls, ls->tk_lexeme->src)) {
				char_err_rdrprint(CHAR_ERR_T_SYNTAX, ls->rdr, 0, CHAR_ERR_LBL_RDEC, ls->tk_lexeme->src);
				char_lex_close(ls);
				return;
			}
	...
	while(true) {
		/*
		if(!char_parser_next(vm->p)) {
			if(vm->p->err)
				char_err_rdrprint(CHAR_ERR_T_SYNTAX, vm->ls->rdr, 0, vm->p->err);
			break;
		}
		*/
		...
		if(!char_vm_runstmt(vm)) {
			if(vm->err)
				char_err_rdrprint(CHAR_ERR_T_RUNTIME, vm->ls->rdr, 0, vm->err);
			break;
		}
	}
